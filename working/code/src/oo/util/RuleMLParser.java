...
class RuleML91Parser {
     ...
     */
     ...
     //Checking to see if assert is the first thing
     //Then checking to see if the next thing is And
     //Then checks to see if there is universal inner closure on the And
     ...
     */
     ...
     //We know the name of the element by checking it then we have to pass the
     //element to the approiate parse method.
     //In the case of assert it can only contain one child
    ...
    private Term parseAssert(Element ass) throws ParseException {
		...
		//if the first child element is an atom get the atom and make it a clause
		//add all atoms to a vector
		//creating a new term with a symbol Assert and Role ("")
		//else
		//if the first child element is Implies
		//call praseImplies
		//create a clause
		//add the atoms to a vector
		//create a new term with symbol "assert" and role ("")
	...
	//Making a new hastable and vector
	//A neg must contain 1 child element
    ...
    private Term parseData(Element data) throws ParseException {
      ...
      //Attribute type = data.getAttribute(XSITYPE,"http://www.w3.org/2001/XMLSchema-instance");
    	...
    	t1.setData(true);
    	System.out.println("PARSING DATA NOW");
    	return t1;
    }
     ...
     */
     ...
     //checking to see if the variable exists in the symbol talbe
     //if not make a new symbol id
     //check to see if it has been used before
     ...
     */
     ...
     //putting all the subterms into a vector
     //then returning a term
     ...
     */
     ...
     //Checking to make CTOR is the first Child
     //Then checking the type
     //Then adding the subterms into a vector
     ...
     */
     ...
     //Check to see if the first element is rel of an atom
     //Store the Rel in the symbol table
     //Get the sub terms of the atom
     ...
     */
     ...
     //A slot must have 2 children
     //The slot name must be constant <slot><Ind>Name</Ind><Ind>Ben</Ind></slot>
     ...
     */
     ...
     //Check to see if it only has one child
     //Then check to see if the next term is a atom or an and
     //If its Atom we parse the child
     //Else if its And we have to parse each child
	...
	//Need to learn more about skolem constants
