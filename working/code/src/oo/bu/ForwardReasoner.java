...
public class ForwardReasoner {
     ...
     */
    public String printClauses(int type) {
    	...
    	if(!flip){
        	flip = true;
           	...
           	while (it.hasNext()) {
            	...
            	stringsRULEML.addElement(dc.toRuleMLString());
        ...
        while (keys.hasMoreElements()) {
        	...
        	if(type == RuleMLParser.RULEML88 || type == RuleMLParser.RULEML91){
        		...
        		while (it.hasNext()) {
                    ...
                    while(iter.hasNext()){
              		String test = (String)iter.next();
                		if(test.equals(dc.toRuleMLString())){
                			print = false;
                		}
                }
    			
    				if(print){
    					out = out + dc.toRuleMLString() + "\n";   
                	}
           		}
		...
		while(e.hasMoreElements()){	
			...
			while(it.hasNext()){
				...
				for(int j = 1; j < dc.atoms.length; j++){
					...
					Edge e1 = null;
					if(hasNegEdge){
					e1 = new Edge(source,sink,negative);
					}
					if(!hasNegEdge){
					e1 = new Edge(source,sink,positive);	
					}
					dg.addEdge(e1);
					edges.addElement(e1);
				}//each body clause	
			...
			for(int i =0; i < numberOfNodes; i++){
				...
				for(int j = 0; j < numberOfNodes;j++){
					...
					if(u.unify(source,t1) && (u.unify(sink,t2))){
						...
						if((Integer)edge.getWeight() == negative){
