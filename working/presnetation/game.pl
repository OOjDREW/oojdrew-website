...
init_facts :- 
			  ...
			  assertz(turned_off(flashlight)),
		        assertz(turned_on(intilaize)),
			  
			  assertz(location(safe, cellar)),
			  assertz(location(gem, safe)),
...
puzzle(goto(cellar)):-
  write('It''s dark and you are afraid of the dark.'),nl,
  !, fail.
...
puzzle(goto('dining room')):-
  write('The door to the dining room is looked'),nl,
  !, fail.  
...
do(look_in(Object)) :- look_in(Object),!.
do(turn_on_flashlight) :- turn_on_flashlight,!.
do(take(X)) :- take(X), !.
do(drop(Item)) :- drop(Item),!.

do(end).
do(_) :- write('Invalid command').
  
command_loop:- 
 write('Welcome to Gem Search'), nl,
  repeat,
  write('<Command Input> '),
  read(X),
  puzzle(X),
...
end_condition(_) :-
  have(gem),
  write('Congratulations You found the gem you win').
